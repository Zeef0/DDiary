...
class Home(ListView):
    ...
    def get_queryset(self):
        ...
        qs = ""
        qs = Entry.objects.filter(Q(privacy="Public")| Q(owner__id= self.request.user.pk)).order_by("?")
...
class EntryDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        entry = Entry.objects.get(slug=self.kwargs.get('slug'))
        context["comment"] = Comment.objects.all().filter(entry=entry)
        content = context
        return content
